...
export default function Header() {
  ...
  const [authUser, setAuthUser] = useState<{ email: string; initials: string; role?: string } | null>(null)
    ...
    async function checkAuth() {
      try {
        ...
        if (user) {
          ...
          // Try to get role from profile API
          let role = 'customer'
          try {
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
          ...
          <Link href="/" className="shrink-0 flex items-center gap-2">
            ...
            <span className="font-800 text-slate-900 dark:text-white text-xl hidden sm:block">
              SMS<span className="text-blue-600 dark:text-blue-400">Tech</span> <span className="text-blue-600 dark:text-blue-400 text-lg font-700">BD</span>
            </span>
          </Link>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {/* Admin Link - only visible to admin/owner users */}
            ...
            <Link href={authUser ? "/account" : "/login"} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title={authUser ? "My Account" : "Sign In"}>
              {authUser ? (

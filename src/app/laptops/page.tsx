...
function LaptopsContent() {
      ...
      try {
        ...
        if (json.success && Array.isArray(json.data)) {
          const mapped = json.data
            ...
            .filter((p: Product) => p.category === 'laptop' || p.subcategory === 'macbook' || p.subcategory === 'gaming' || p.subcategory === 'pre-owned' || p.badges.includes('Pre-Owned'))
          setProducts(mapped)
        }
      } catch (e) {
  ...
  const availableSubcategories = ['gaming', 'business', 'student', 'creator', 'ultrabook', 'macbook', 'pre-owned']
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <nav className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-6">
        ...
        <span>›</span>
        <span className="text-slate-900 dark:text-white font-600">Laptops</span>
        {catParam && <><span>›</span><span className="text-slate-900 dark:text-white font-600 capitalize">{catParam}</span></>}
      </nav>

      <div className="mb-6">
        <h1 className="text-3xl font-800 text-slate-900 dark:text-white">Laptop Price in Bangladesh</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1.5 leading-relaxed">
          Laptop Price starts from BDT 27,900 to BDT 649,900 in Bangladesh, depending on Brand, Specifications, and Features. Buy original branded laptop from SMSTech Laptop shop in BD. Browse below and order yours now!
        </p>
      </div>

      {/* Top Bar with embedded Brand Selector Pills, Product Count, Sort & Share Link */}
      <ProductTopBar
        totalCount={sortedAndFiltered.length}
        availableBrands={availableBrands.length > 0 ? availableBrands : ['Apple', 'ASUS', 'Lenovo', 'HP', 'Dell']}
        ...
        <div className="flex-1 w-full">
          {loading ? (
            <div className="text-center py-20 text-slate-500 dark:text-slate-400">
              ...
              Loading laptops from database...
            </div>
          ) : sortedAndFiltered.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
              <p className="text-lg font-600 text-slate-700 dark:text-slate-200">No laptops match your selected filters</p>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Try clearing some filters or changing your search criteria.</p>
              <Link href="/laptops" className="inline-block mt-4 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-600">Reset Filters</Link>
            </div>
          ) : (
...
export default function Laptops() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-slate-500 dark:text-slate-400">Loading laptops...</div>}>
      <LaptopsContent />
    </Suspense>
  )
}

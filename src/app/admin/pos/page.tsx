...
export default function PosTerminalPage() {
  ...
  // Fetch Active Shift when branch changes
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Ignore if user is actively typing in text fields other than rapid scanner
      // target removed
      // isInput check
      ...
      if (e.key === "Enter") {
        if (barcodeBuffer.current.length >= 4) {
          e.preventDefault();
          executeInstantScan(barcodeBuffer.current);
          barcodeBuffer.current = "";
        }
  ...
  const addDeviceUnitToCart = (unit: any) => {
    ...
    if (cart.some(i => i.deviceUnitId === unit.id)) {
      alert("This serialized unit is already in the cart!");
      return;
    }
  ...
  const addProductToCart = (product: any) => {
    const existing = cart.find(i => !i.isSerialized && i.productId === product.id);
    if (existing) {
      setCart(cart.map(i => i.cartId === existing.cartId ? { ...i, quantity: i.quantity + 1 } : i));
    } else {
      const newItem: PosItem = {
  ...
  const openCheckout = () => {
    if (!activeShift) {
      alert("Please open a register shift first!");
      setShowOpenShiftModal(true);
      return;
    }
    if (cart.length === 0) {
  ...
  const handleCheckoutSubmit = async () => {
    const totalTendered = splitPayments.reduce((s, p) => s + Number(p.amount), 0);
    if (Math.abs(totalTendered - finalTotal) > 0.01) {
      alert(`Split payment total (৳${totalTendered}) must equal the final bill total (৳${finalTotal})`);
      return;
    }
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
              ...
              {splitPayments.map((p, idx) => (
                <div key={idx} className="flex items-center gap-2 p-2 bg-zinc-50 dark:bg-zinc-800 rounded-xl">
                  ...
                  >
                    ...
                    <option value="due">Create Due Balance</option>
                  </select>
            ...
            <div className="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl flex items-center justify-between text-xs font-bold">
              <span>Total Tendered: {fmt(splitPayments.reduce((s, p) => s + Number(p.amount), 0))}</span>
              <span className={Math.abs(splitPayments.reduce((s, p) => s + Number(p.amount), 0) - finalTotal) < 0.01 ? "text-emerald-600" : "text-rose-600"}>
                {Math.abs(splitPayments.reduce((s, p) => s + Number(p.amount), 0) - finalTotal) < 0.01 ? "✓ Tender Balanced" : `Difference: ${fmt(finalTotal - splitPayments.reduce((s, p) => s + Number(p.amount), 0))}`}
              </span>
            </div>
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-extrabold shadow-lg shadow-emerald-600/30 transition-all disabled:opacity-50"
              >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-1.5 p-3 bg-zinc-50 dark:bg-zinc-800/60 rounded-xl text-xs">
              ...
              <div className="flex justify-between"><span>Card Sales:</span><span className="font-bold">{fmt(activeShift.card_sales_total)}</span></div>
              <div className="flex justify-between"><span>Mobile (bKash/Nagad):</span><span className="font-bold">{fmt(activeShift.mobile_sales_total)}</span></div>
              <div className="flex justify-between pt-1 border-t border-zinc-200 dark:border-zinc-700 font-extrabold">
                <span>Expected Drawer Cash:</span>
                <span className="text-blue-600">{fmt(Number(activeShift.opening_float) + Number(activeShift.cash_sales_total))}</span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={async () => {
                  ...
                  const json = await res.json();
                  if (json.success) {
                    alert(`Shift closed! Difference: ৳${json.data.difference}`);
                    setActiveShift(null);
